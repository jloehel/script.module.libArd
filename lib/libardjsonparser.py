        ...
        for entry in j2:
            ...
            for j3 in entry["inhalte"]:
                if runtimeToInt(j3["unterzeile"]) > duration:
                    ...
                    d["_thumb"] = j3["bilder"][0]["schemaUrl"].replace(
                        "##width##", "384")
                    d["url"] = j3["link"]["url"]
                    d["documentId"] = j3["link"]["url"].split(
                        "player/")[1].split("?")[0]
    ...
    for entry in j1:
        ...
        if True:
            for entry in entry["inhalte"]:
                d = {}
                d["_name"] = entry["ueberschrift"]
                d["_channel"] = entry["unterzeile"]
                d["_entries"] = int(
                    entry["dachzeile"].split(' ')[0])
                # d["thumb"] = entry["bilder"][0]["schemaUrl"].replace("##width##","0")
                d["_thumb"] = entry["bilder"][0]["schemaUrl"].replace(
                    "##width##", "1920")
                d["url"] = entry["link"]["url"]
...
def parseVideos(url):
    ...
    for j2 in j1["inhalte"]:
        ...
        if "ueberschrift" in j2:
            d["_name"] = j2["ueberschrift"]
            if 'Hörfassung' in d["_name"] or 'Audiodeskription' in d["_name"]:
                d["_name"] = d["_name"].replace(
                    ' - Hörfassung', '').replace(' - Audiodeskription', '')
                d["_name"] = d["_name"].replace(' (mit Hörfassung)', '').replace(
                    ' (mit Audiodeskription)', '')
                d["_name"] = d["_name"].replace(
                    ' mit Hörfassung', '').replace(' mit Audiodeskription', '')
                d["_name"] = d["_name"].replace(
                    ' (Hörfassung)', '').replace(' (Audiodeskription)', '')
                d["_name"] = d["_name"].replace(
                    ' Hörfassung', '').replace(' Audiodeskription', '')
        ...
        if "bilder" in j2:
            d["_thumb"] = j2["bilder"][0]["schemaUrl"].replace(
                "##width##", "384")
        if "teaserTyp" in j2:
            if j2["teaserTyp"] == "PermanentLivestreamClip" or j2["teaserTyp"] == "PodcastClip":
                ...
                libMediathek.log(
                    'json parser: unknown item type: ' + j2["teaserTyp"])
                d["_type"] = 'dir'
                d['mode'] = 'libArdListVideos'

        if "link" in j2:
            d["url"] = j2["link"]["url"]
            d["documentId"] = j2["link"]["url"].split(
                "/player/")[-1].split("?")[0]
        if "dachzeile" in j2:
            d["_releasedate"] = j2["dachzeile"]
...
def runtimeToInt(runtime):
    try:
        ...
        t = runtime.replace('Min', '').replace('min', '').replace(
            '.', '').replace(' ', '').replace('|', '').replace('UT', '')
        HHMM = t.split(':')
        if len(HHMM) == 1:
