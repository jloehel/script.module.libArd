...
def getVideoUrl(url=False, videoID=False):
    ...
    if url:
        ...
        match = re.compile(
            '<div class="box fsk.*?class="teasertext">(.+?)</p>', re.DOTALL).findall(content)
    # if match:
    if False:
        xbmc.executebuiltin(
            'XBMC.Notification(Info:,'+match[0].strip()+',15000)')
        return False
    else:
...
def fetchTvaVideo(id):
    xml = libMediathek.getUrl(
        'http://www.ardmediathek.de/ard/servlet/export/tva/id='+id+'/index.xml')
    ...
    match = re.compile(
        '<tva:OnDemandProgram>(.+?)</tva:OnDemandProgram>', re.DOTALL).findall(xml)
    finalUrl = False
    qualityHLS = 0
    for item in match:
        videoUrl = re.compile(
            '<tva:ProgramURL>(.+?)</tva:ProgramURL>', re.DOTALL).findall(item)[0]
        if not 'rtmp://' in videoUrl and not 'rtmpt://' in videoUrl and not 'manifest.f4m' in videoUrl:
            quality = re.compile(
                '<tva:FileFormat href="urn:ard:tva:metadata:cs:ARDFormatCS:(.+?)"/>', re.DOTALL).findall(item)[0]
    ...
    metadata = {}
    try:
        metadata['name'] = re.compile(
            '<tva:Title type="main">(.+?)</tva:Title>', re.DOTALL).findall(xml)[0]
    except:
        pass
    try:
        s = xml.split(
            '<tva:Format href="urn:ard:tva:metadata:cs:ARDFormatCS:2014:4.4.11.0.6"/>')[1]
        metadata['thumb'] = re.compile(
            '<mpeg7:MediaUri>(.+?)</mpeg7:MediaUri>', re.DOTALL).findall(s)[0]
        metadata['plot'] = re.compile(
            '<ard:alternativeText>(.+?)</ard:alternativeText>', re.DOTALL).findall(s)[0]
        metadata['name'] = re.compile(
            '<tva:Title type="main">(.+?)</tva:Title>', re.DOTALL).findall(xml)[0]
    except:
        pass
